...
#pragma omp parallel for collapse(3)
			for (int64_t x = 0; x < res.getSizeX(); ++x)
				for (int64_t y = 0; y < res.getSizeY(); ++y)
					for (int64_t z = 0; z < res.getSizeZ(); ++z)
					{
						T currentMax = min;
						const TOffset offsetRes = im.getOffset(x + back[0], y + back[1], z + back[2]);
						for (auto itSe = se.begin(); itSe != endSe; ++itSe)
						{
							TOffset offsetIm = offsetRes + *itSe;
							currentMax = std::max(currentMax, im(offsetIm));
						}
						res(x, y, z) = currentMax;
					}
		}
...
#pragma omp parallel for collapse(3)
	...
	template <class T>
	Image<T> dilationNoBorders(Image<T> im, FlatSE se)
	{
		Image<T> res = im;
...
#pragma omp parallel for collapse(3)
	...
	template <class T>
	Image<T> erosionNoBorder(Image<T> im, FlatSE se)
	{
		Image<T> res = im;
		se.setContext(im.getSize());
...
#pragma omp parallel for collapse(3)
	...
	**/
	...
	template <class T>
	Image<T> openingNoBorder(Image<T> im, FlatSE se)
	{
		return dilationNoBorders(erosionNoBorder(im, se), se);
	}
	...
	**/
	...
	template <class T>
	Image<T> closingNoBorder(Image<T> im, FlatSE se)
	{
		return erosionNoBorder(dilationNoBorders(im, se), se);
	}
	...
	**/
	...
	{
		Image<T> tmp = erosionNoBorder(im, se);
		im = dilationNoBorders(im, se);
		im -= tmp;
		return im;
	}
